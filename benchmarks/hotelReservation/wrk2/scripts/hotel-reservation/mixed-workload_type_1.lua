...
local data ="http://10.109.208.73:5000/"
...
local function search_hotel() 
  ...
  local method = "GET"
  local path = data .. "hotels?inDate=" .. in_date_str .. 
...
local function recommend()
  ...
  local method = "GET"
  local path = data .. "recommendations?require=" .. req_param .. 
...
local function reserve()
  ...
  local method = "POST"
  local path = data .. "reservation?inDate=" .. in_date_str .. 
    "&outDate=" .. out_date_str .. "&lat=" .. tostring(lat) .. "&lon=" .. tostring(lon) ..
...
local function user_login()
  ...
  local method = "GET"
  local path = data .. "user?username=" .. user_name .. "&password=" .. password
  local headers = {}
  -- headers["Content-Type"] = "application/x-www-form-urlencoded"
  return wrk.format(method, path, headers, nil)
end
